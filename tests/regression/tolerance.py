...
Uses best-match event pairing (Hungarian-style greedy) to avoid false
failures from event reordering, and ``rapidfuzz.fuzz.token_set_ratio``
...
def _best_match_pairs(
    ...
    """Pair actual events with expected events by minimizing total distance.

    Uses a greedy algorithm: for each expected event, find the closest
    unmatched actual event.  Returns a list of (actual, expected, distance)
    tuples.  Unmatched expected events are not included.
...
def assert_extraction_result(
    ...
    for actual_event, expected_event, _dist in pairs:
        ...
        # Existing event ID required check.
        if expected_event.existing_event_id_required and actual_event.existing_event_id is None:
            errors.append(
